...
// Skeleton loader for context
const ContextSkeleton = () => (
    <div className="flex items-center gap-2 animate-pulse">
        <div className="flex gap-1">
            <span className="w-2 h-2 bg-purple-400/50 rounded-full animate-bounce" style={{ animationDelay: '0ms' }}></span>
            <span className="w-2 h-2 bg-purple-400/50 rounded-full animate-bounce" style={{ animationDelay: '150ms' }}></span>
            <span className="w-2 h-2 bg-purple-400/50 rounded-full animate-bounce" style={{ animationDelay: '300ms' }}></span>
        </div>
        <span className="text-xs text-purple-300/70 italic">Generating context...</span>
    </div>
);

// Context display with fade-in animation
const ContextDisplay = ({ context }) => (
    <p className="text-sm text-purple-300 mt-2 italic border-l-2 border-purple-500/30 pl-3 animate-fadeIn">
        "{context}"
    </p>
);
...
const ResultTimeline = ({ path }) => {
    ...
    const getPageData = (item) => {
        // New format: {node: {...}, edge_context: ...}
        if (item.node) {
            return item.node;
        }
        // Old format: direct page object
        return item;
    };

    const getEdgeContext = (item) => {
        // New format: {node: {...}, edge_context: ...}
        if (item.hasOwnProperty('edge_context')) {
            return item.edge_context;
        }
        // Old format: no context
        return undefined;
    };
                    ...
                    return (
                        ...
                        >
                            ...
                            >
                                ...
                                {/* Edge Context - Shows between nodes */}
                                {index < path.length - 1 && (
